...
async function formatRankCard(client, canvas, message) {
    ...
    client.connection.query(`SELECT * FROM members`, async (error, rows) => {
        ...
        context.fillText(`${expSimple}/${expForNextLvl}`, canvas.width - padding * 2 - 98, canvas.height - padding * 2 - 5);
        context.fillText(`${message.author.tag}`, padding * 2 + 5, padding * 2 + avatar.height + 40);
        context.fillText('Ваш рейтинг:', padding * 2 + 5 + avatar.width + 35, padding * 2 + 28);
        context.fillStyle = "rgb(255,255,255)";
        context.font = '44px sans-serif';
        context.fillText(`#${indexAuthor + 1}`, padding * 2 + 5 + avatar.width + 200, padding * 2 + 33);

        context.fillStyle = "rgb(180,180,180)";
        context.font = '28px sans-serif';
        context.fillText('Рівень: ', padding * 2 + 5 + avatar.width + 315, padding * 2 + 28)
        context.fillStyle = "rgb(255,255,255)";
        context.font = '44px sans-serif';
        context.fillText(`${sortedArr[indexAuthor].level}`, padding * 2 + 5 + avatar.width + 410, padding * 2 + 33)
